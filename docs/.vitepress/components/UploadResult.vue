...
const pagination = reactive({
  page: 1,
  pageSize: computed(() => keyword.value ? 99999 : 10),
  showSizePicker: true,
                ...
                return h(NSpace, { vertical: true, size: 'small' }, {
                  default: () =>
                    [
                      ...
                      { label: '上传开始时间', value: data.upload_begin_time || '暂无' },
                      { label: '上传成功时间', value: data.upload_success_time || '暂无' },
                      { label: '下次重试时间', value: data.upload_locker_expire || '暂无' },
                      { label: '上传节点', value: `UploadServer-${data.upload_node?.slice(0, 15).toUpperCase() || '未知'}` }
                    ].map(item =>
